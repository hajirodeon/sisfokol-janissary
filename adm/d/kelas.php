...
if ($s == "edit")
	...
	//query
	$qnil = mysql_query("SELECT * FROM m_kelas ".
				"WHERE kd = '$pelkd'");
	...
	if (empty($kelas))
		...
		if ($s == "edit")
			...
			if ($tcc > 1)
				...
				{
				//update
				mysql_query("UPDATE m_kelas SET kelas = '$kelas' ".
								"WHERE kd = '$pelkd'");
		...
		if (empty($s))
			...
			if ($tcc != 0)
				...
				{
				//insert data
				mysql_query("INSERT INTO m_kelas(kd, kelas) VALUES ".
						"('$x', '$kelas')");
...
<td>
Nama Kelas :
<input name="kelas" type="text" value="'.$y_kelas.'" size="20" '.$x_enter.'>
...
//query
$qdata = mysql_query("SELECT * FROM m_kelas ".
			"ORDER BY kelas ASC");
...
if ($tdata != 0)
	...
	<td width="1">&nbsp;</td>
	<td width="1">&nbsp;</td>
	<td valign="top"><strong>Mata Kelas</strong></td>
	</tr>';
	...
	<tr>
	<td>
	<input type="button" name="Button" value="SEMUA" onClick="checkAll('.$tdata.')">
